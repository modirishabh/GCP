...
- **Integrity Monitoring** establishes a baseline hash of the boot process in a trusted state.  
- On each reboot, the system verifies the boot sequence against that baseline to detect tampering.  
- After system updates that alter files, update the integrity policy baseline before rebooting to avoid false alerts.
